...
epsilon_LM = 0.5  # Modulation factor for landmark cells
delta_phi = 15  # Phase difference for EC and WC cells
v = 5 # Velocity of the agent
...
# Function to update weights in Hebbian way
...
for step in range(steps):
    ...
    elif step >= 2000  and step < 3000:
        s_EC = np.zeros_like(s)
        s_WC = (v/v_c0) * np.roll(s, int(delta_phi))
    elif step >= 3000 and step < 4000:
        s_EC = (v/v_c0) * np.roll(s, -int(delta_phi))
        s_WC = np.zeros_like(s)
    elif step >= 4000 and step < 5000:
        s_EC = np.zeros_like(s)
        s_WC = (v/v_c0) * np.roll(s, int(delta_phi))
    else:
        s_EC = np.zeros_like(s)
...
np.save(os.path.join('./simulation/data/','full_velocity_activations.npy'), np.array(activation_history))
np.save(os.path.join('./simulation/data/','full_activations_ec.npy'), np.array(ec_activation_history))
np.save(os.path.join('./simulation/data/','full_activations_wc.npy'), np.array(wc_activation_history))
np.save(os.path.join('./simulation/data/','full_w1_history.npy'), np.array(w1_history))
np.save(os.path.join('./simulation/data/','full_w2_history.npy'), np.array(w2_history))
np.save(os.path.join('./simulation/data/','full_landmark_1.npy'), np.array(s_L1))
np.save(os.path.join('./simulation/data/','full_landmark_2.npy'), np.array(s_L2))
