...
class RingAttractorAnimation(Scene):
    ...
    def construct(self):
        ...
        landmark_1_data = np.load(os.path.join('./simulation/data/','landmark_1.npy'))
        landmark_2_data = np.load(os.path.join('./simulation/data/','landmark_2.npy'))
        ...
        movement_line = Line(LEFT * 4 + DOWN * 3.5, RIGHT * 4 + DOWN * 3.5, color=GRAY, stroke_opacity=0.1)
        self.add(movement_line)

        # Create boxes at each end of the line
        box_size = 0.5  # Size of the square box, change as needed
        left_box = Square(side_length=box_size, color=ORANGE, fill_opacity=0.1, stroke_opacity=0.2)
        right_box = Square(side_length=box_size, color=ORANGE, fill_opacity=0.1, stroke_opacity=0.2)
        ...
        mouse = Circle(radius=0.2, stroke_color=GREY_BROWN, fill_color=GREY_BROWN, fill_opacity=0.2, stroke_opacity=0.2)
        mouse_start = RIGHT * 4 + DOWN * 3.5  # Start position on the screen near the bottom
        ...
        self.play(
            ...
            *[ec_ball.animate.set_stroke(opacity=0.2) for ec_ball in balls_ec],
            ...
            left_box.animate.set_fill(opacity=0.5),
            left_box.animate.set_stroke(opacity=1),
            right_box.animate.set_fill(opacity=0.5),
            right_box.animate.set_stroke(opacity=1),
            movement_line.animate.set_stroke(opacity=0.7),
            mouse.animate.set_fill(opacity=1),
            mouse.animate.set_stroke(opacity=1),
            run_time=2
        )
        ...
        # Animate activity based on data
        step_size = 10  # Update only every 10 steps
        for step in range(0, len(activity_data), step_size):
            ...
            for i, (w1, w2, arrow1, arrow2) in enumerate(zip(w1_selected, w2_selected, arrs_1, arrs_2)):
                index = i * 5 if i < len(balls) else i  # Ensure index mapping for main balls
                ball = balls[i]
                ...
                animations.append(ball.animate.set_fill(ball_color, opacity=1 if normalized_activity > 0 else 0))
                animations.append(landmark_2.animate.set_fill(landmark2_color, opacity=1))

                norm_w1 = w1 / np.max(w1_data) if np.max(w1_data) > 0 else 0
                norm_w2 = w2 / np.max(w2_data) if np.max(w2_data) > 0 else 0
                norm_w1 = 0 #if norm_w1 < 0.3 else norm_w1
                norm_w2 = 0 if norm_w2 < 0.3 else norm_w2
        ...
        self.wait(5)  # Hold the final scene for a moment before closing